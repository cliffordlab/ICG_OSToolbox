...
% ----------  Impedance Cardiogram Manual Annotation Application ---------
% OVERVIEW:
...
%         limit_ch2_Y2 = (maxamp_Ch2Ca+0.1*abs(maxamp_Ch2Ca));
        ...
        text(hplotICG, t(Rpeak_indices),limit_ann*ones(1,length(Rpeak_indices)),'R','color','red','Clipping','on')
        text(hplotICG, t(Cpoint_indices),limit_ann*ones(1,length(Cpoint_indices)),'C','color','black','Clipping','on')
        text(hplotICG, t(Bpoint_indices),limit_ann*ones(1,length(Bpoint_indices)),'B','color','blue','Clipping','on')
        text(hplotICG, t(Xpoint_indices),limit_ann*ones(1,length(Xpoint_indices)),'X','color','magenta','Clipping','on')
...
txt_timeaxes = uicontrol(hMainFigure, ...
    ...
    function time_con(hObject, ~)
        ...
        if exist( strcat(folder_path_manual,record_name,'_CShape','.txt') , 'file')
            ...
            beatsCshape = 1.*ones(length(Rpeak_indices),1);               % Initializing to "Select C shape"    
        end
        ...
        % parameter panel initialized
                ...
                set(hdisp_C_Shape,'String',strcat('C: ', C_shape));
                set(hdisp_C_Shape, 'Position', [Cnew_st_x-1.9*w1    0.57    0.13  highButtons])
        ...
        elseif (strcmp(timelim,'RC Interval'))
            ...
            if (length(Rpeak_indices) <=1) || (length(Cpoint_indices) <=1)
                ...
                C_shape = C_shp{ beatsCshape(1)};
                set(hdisp_C_Shape,'String',strcat('C: ', C_shape));
                set(hdisp_C_Shape, 'Position', [Cnew_st_x-1.9*w1    0.20    0.13 highButtons])
...
hRC_pop = uicontrol(hMainFigure, ...          % popup menu for "RC features"
    ...
    'String', {'Select RC feature','Notch','Valley','Plateau','Inflection','Mild inflection','Change in gradient',...
    ...
    function RC_con(hObject, ~)
        ...
        elseif (strcmp(RC_feat,'Notch'))
            ...
            inflec_clear
            add_Bpoint;
        elseif (strcmp(RC_feat,'Change in gradient'))
            RC_feature = 'Change in gradient';
            inflec_clear;
            add_Bpoint;
    ...
    function add_autoBpoint(~,~)
        ...
        if (length(Rpeak_indices_sorted) ~= length(Cpoint_indices_sorted)) || (length(Rpeak_indices_sorted) <=1) || (length(Cpoint_indices_sorted) <=1)
           errordlg('Number of R peaks are not equal to number of C points:  Please verify and annotate','RC Calculation Error','warn');
            ...
            low_limit = Rpeak_indices_sorted(page_cur)+35;
            upper_limit = Rpeak_indices_sorted(page_cur)+130;   % RB restriction of lower and upper limit
            rg_4_B = [low_limit upper_limit];
            
            C_75mag = 0.75*icg(Cpoint_indices_sorted(page_cur));           % .75*C point_ amplitude  

            icg_mag = icg(rg_4_B(1):rg_4_B(2));

            Index_C_75 = find(icg_mag < C_75mag);
            upper_limit = low_limit + length(Index_C_75);
            rg_4_Bmod = [low_limit   upper_limit];
         
            icg_3_der= icg_D2(rg_4_Bmod(1):rg_4_Bmod(2));
           
            [~, Index] = max(icg_3_der);
            
            xt = (rg_4_Bmod(1) +Index)/1000;                     % just to go with code
...
hX_pop = uicontrol(hMainFigure, ...          % popup menu for "X shape"
    'Style', 'popupmenu', ...?% can changed text
    'HandleVisibility', 'callback', ...
    'Units', 'normalized',...
