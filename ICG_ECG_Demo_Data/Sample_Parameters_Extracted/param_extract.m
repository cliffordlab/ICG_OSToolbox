...
%    5. Amplitude of B point and X point.
...
Bamp = struct();                     % B amplitude for all beats of all records
Xamp = struct();                     % X amplitude for all beats of all records
...
for k = 1 : length(EAFiles)         
  ...
  Bamp(k).ID = basename(1:end-4);  
  Xamp(k).ID = basename(1:end-4);  
  ...
  if no_Rpk == 0
    ...
    Bamp(k).B_amp= B_amp;
    Bamp(k).Bamp_Avg = Bamp_Avg; 
    RB(k).RB= RBs;
    RB(k).RB_Avg= RB_Avg;        
    Record_avg(k).Bamp_Avg = Bamp_Avg; 
 ...
 if no_Cpoint == 0
  ...
  if no_Xpoint == 0
      ...
      Xamp(k).Xamp = EA_icg(Xpoint);
      Xamp(k).Xamp_Avg = mean(Xamp(k).Xamp);
      BX(k).BX = BXs;
      BX(k).BX_Avg = BX_Avg;
      Record_avg(k).Xamp_Avg = mean(Xamp(k).Xamp);
...
disp('Saving B amplitudes for beats of all records ...')  
writetable(struct2table(Bamp), strcat(EA_folder, 'Demo_Bamp_beat_parameters.xlsx'))
disp('Saving X amplitudes for beats of all records ...')  
writetable(struct2table(Xamp), strcat(EA_folder, 'Demo_Xamp_beat_parameters.xlsx'))
